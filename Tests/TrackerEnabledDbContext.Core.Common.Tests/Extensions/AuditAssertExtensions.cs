...
namespace TrackerEnabledDbContext.Core.Common.Tests.Extensions
{
    public static class AuditAssertExtensions
    {
        public static T AssertAuditForAddition<T>(this T entity, ITrackerContext db, object entityId, string userName = null, params Expression<Func<T, object>>[] propertyExpressions)
        {
            ...
            foreach (var expression in propertyExpressions)
            {
                ...
                lastLog.LogDetails.AssertAny(x => x.NewValue == keyValuePair.Value && x.PropertyName == keyValuePair.Key);
            }
        ...
        public static T AssertAuditForSoftDeletion<T>(this T entity, ITrackerContext db, object entityId, string userName = null, params AuditLogDetail[] logdetails)
        {
            return AssertChange(entity, db, entityId, userName, EventType.SoftDeleted, logdetails);
        }

        public static T AssertAuditForUndeletion<T>(this T entity, ITrackerContext db, object entityId, string userName = null, params AuditLogDetail[] logdetails)
        {
